...
class DlgFloraAdapter(var context: Context, var itemList : ArrayList<Floras>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent : ViewGroup?): View {
        ...
        var Floras : Floras = getItem(position)

        viewHoldar.search_item.text = Floras.name_kr;

        if(Floras.chkSelect == true){
    ...
    override fun getItem(position: Int): Floras {
...
/*        for(i in 0.. (itemList.size-1)){
        ...
        var Floras:Floras = itemList.get(selectIndex);
        Floras.chkSelect = false
        itemList.set(selectIndex,Floras)
