...
class DlgInsectAdapter(var context: Context, var itemList : ArrayList<Insects>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent : ViewGroup?): View {
        ...
        var Insects : Insects = getItem(position)

        viewHoldar.search_item.text = Insects.name_kr;

        if(Insects.chkSelect == true){
    ...
    override fun getItem(position: Int): Insects {
    ...
    fun addItem(Insects: Insects){

        itemList.add(Insects);
        notifyDataSetChanged()
    }
...
/*        for(i in 0.. (itemList.size-1)){
        ...
        var Insects:Insects = itemList.get(selectIndex);
        Insects.chkSelect = false
        itemList.set(selectIndex,Insects)
